...
import {
  ...
  Clock, BarChart3, Settings, ArrowUp, ArrowDown, ChevronRight, Eye
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const statCards = [
    {
      ...
      bg: "bg-brand-500/10",
      change: ch?.users.pct ? `${ch.users.pct}%` : null,
      up: ch?.users.up ?? true,
    },
    {
      label: "Active Writers",
      value: stats?.writerStats?.approved?.toLocaleString() ?? "—",
      icon: FileText,
    ...
    },
    {
      ...
      change: ch?.revenue.pct ? `${ch.revenue.pct}%` : null,
      up: ch?.revenue.up ?? true,
    },
    {
  ...
  const alertCards = [
    ...
    { label: "Open Disputes",   value: stats?.disputedOrders ?? 0, icon: AlertCircle, color: "text-red-400",    href: "/admin/disputes" },
  ];

  // Real chart data from DB, formatted for display
  const chartData = (stats?.revenueByDay ?? []).map((d: { date: string; revenue: number; orders: number }) => ({
    day: d.date,
    revenue: d.revenue,
    orders: d.orders,
  }));

  const hasRevenue = chartData.some((d: { revenue: number }) => d.revenue > 0);

  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl lg:text-3xl font-bold text-white">Admin Dashboard</h1>
        <p className="text-slate-400 mt-1">Platform overview — all numbers are live from the database</p>
      </div>
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              {card.change ? (
                <div className={`flex items-center gap-0.5 text-xs font-semibold ${card.up ? "text-green-400" : "text-red-400"}`}>
                  ...
                  {card.change}
                  <span className="text-gray-600 ml-0.5">7d</span>
                </div>
              ) : <span />}
            </div>
            <div className="text-xl lg:text-2xl font-bold text-white mb-0.5">
              {isLoading ? <span className="text-slate-600">—</span> : card.value}
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-white font-semibold">Revenue & Orders (Last 7 Days)</h2>
            {hasRevenue ? (
              <span className="text-xs text-slate-500">Live data</span>
            ) : (
              <span className="text-xs text-slate-600">No revenue recorded yet</span>
            )}
          </div>
          ...
          {hasRevenue ? (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData}>
                ...
                <YAxis stroke="#475569" fontSize={11} tickFormatter={(v) => `$${(v / 100).toFixed(0)}`} />
                <Tooltip
            ...
            <div className="flex flex-col items-center justify-center h-48 text-slate-600 text-sm gap-2">
              ...
              <p>Chart will populate once orders start coming in</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          {[
            ...
            { label: "Content Views",     href: "/admin/analytics",            icon: Eye,         color: "text-purple-400" },
            { label: "System Settings",   href: "/admin/settings",             icon: Settings,    color: "text-slate-400" },
          ].map((action) => (
            <Link key={action.label} href={action.href}
              className="flex items-center gap-3 p-3 glass rounded-xl hover:border-white/20 transition-all group">
      ...
      <div className="mt-8">
        ...
        <div className="glass-card overflow-hidden">
          {recentOrders.length === 0 ? (
            <div className="text-center py-12 text-slate-600 text-sm">No orders yet</div>
          ) : (
